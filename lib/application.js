...
app.use = function use(path, fn) {
  ...
  for (var i = offset; i < args.length; i++) {
    if (typeof args[i].handle === 'function' && args[i].set) {
        ...
        // Use IIFE to avoid bad reference to mount_app in the closure
        var handler = (function(subApp) {
          return function mounted_app(req, res, next) {
            var orig = req.app;
            subApp.handle(req, res, function (err) {
